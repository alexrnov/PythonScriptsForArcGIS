...
def replacement_code_lists(wells_array,def_type_tn,def_status_document,def_type_document):
        ...
        while(k<len(wells_array)):#перебор элементов массива точек (из листа Точки наблюдений)
                ...
                while(m<len(def_type_document)):#перебор кодов типов документирования
                        ...
                        if(str(wells_array[k][10])==str(def_type_document[m][0])):
                                array_wells_replace[k][10]=def_type_document[m][1]#поменять код на расшифровку
                                break#выйти из внутреннего цикла
                        m+=1
                k+=1
...
if __name__== '__main__':#если модуль запущен, а не импортирован
        ...
        output_workspace = ConversionUtils.gp.GetParameterAsText(4)#рабочая область
        Full_report = ARCPY.GetParameterAsText(5)#параметр определяет выводить ли полный отчет
        List_point = ARCPY.GetParameterAsText(6)#Название листа с точками наблюдений
        List_strat_lit = ARCPY.GetParameterAsText(7)#Название листа с описанием стратиграфии и литологии
        List_code_strat = ARCPY.GetParameterAsText(8)#Название листа с кодами стратиграфии
        List_code_lit = ARCPY.GetParameterAsText(9)#Название листа с кодами литологии
        List_GIS = ARCPY.GetParameterAsText(10)#Название листа с ГИС
        List_code_type_TN = ARCPY.GetParameterAsText(11)#Название листа с кодами типов ТН
        List_code_status_document = ARCPY.GetParameterAsText(12)#Название листа с кодами состояния документирования
        ...
        for input in excel_list_files:#перебор excel-файлов
                ...
                wells_array_final = replacement_code_lists(wells_array,code_type_tn,code_status_document,code_type_document)
                ...
                write_to_layer(outdata, Coordinate_System, map_array)
