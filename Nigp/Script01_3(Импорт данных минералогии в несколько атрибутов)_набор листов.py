...
import arcgisscripting, ConversionUtils, os, traceback
try:
...
def verify_lists(d_point, d_strat_lit, d_code_strat, d_code_lit, d_mineral, d_code_test, d_type_TN, d_document,d_test,d_type_document):
        ...
        (d_test=="null")or(d_type_document=="null")):return False
        else:return True
...
def calc_non_empty(atributes,values):
        ...
        while(k<len(values)):#перебор строк листа
                ...
                l_code_old = 'no_dada'#код возраста
                l_code_rock = 'no_data'#код породы
                ...
                while(l<len(atributes)):#перебор атрибутов текущей строки
                        ...
                        if(l==6):l_code_old = str(values[k][l])#код возраста
                        if(l==7):l_code_rock = str(values[k][l])#код породы
                        if((l>=8)and(l<len(atributes)-1)):
                                ...
                                if(values[k][l]<>''):
                                        ...
                                        curr_arr = [id_points,number_test,id_test,code_type_test,\
                                        interval_from,interval_to,l_code_old,l_code_rock,stroka_mineral]
                        l+=1
...
#и вместо l-кодов стратиграфии и литологии - расшифрованными значениями
def replacement_code_test(def_non_empty,def_code_type_test,def_code_strat,def_code_lit):
        k=0
        while(k<len(def_non_empty)):#перебор элементов массива точек с минералогией
                ...
                while(m<len(def_code_type_test)):#перебор кодов типов пород
                        ...
                        m+=1
                m=0
                if(def_non_empty[k][6]=='')or(def_non_empty[k][6]==' '):
                        def_non_empty[k][6]='Нет данных'
                else:#если есть значение кода стратиграфии
                        while(m<len(def_code_strat)):#перебор кодов стратиграфии(возраста)
                                #если код типа возраста из массива с минералогией совпал с кодом из листа с кодами стратиграфии пород
                                if(def_non_empty[k][6]==str(def_code_strat[m][0])):
                                        def_non_empty[k][6]=def_code_strat[m][1]#поменять код типа породы на название типа стратиграфии
                                        break#выйти из внутреннего цикла
                                m+=1
                        m=0
                if(def_non_empty[k][7]=='')or(def_non_empty[k][7]==' '):
                        def_non_empty[k][7]='Нет данных'
                else:#если есть значение кода литологии
                        while(m<len(def_code_lit)):
                                #если код типа литологии из массива с минералогией совпал с кодом из листа с кодами литологии
                                if(def_non_empty[k][7]==str(def_code_lit[m][0])):
                                        def_non_empty[k][7]=def_code_lit[m][1]#поменять код литологии на его расшифровку
                                        break
                                m+=1
                k+=1
        #вернуть массив в формате
        #[id ТН, номер пробы, id пробы, тип пробы, интервал от, интервал до, возраст, порода, минералогия]
...
#[id_скважины,линия/имя скважины,x,y,z,глубина скважины,объект,участок,номер пробы,id пробы,код типа пробы,интервал глубины от, интервал глубины до, находки МСА]
	...
	while(k<len(arr_min)):#перебор элементов первого листа (массива точек)
    		...
    		while(m<len(arr_wells)):#перебор элементов второго листа (массива с данными минералогии) 
				...
				curr_array = [arr_wells[m][0],arr_wells[m][1],arr_wells[m][2],arr_wells[m][3],arr_wells[m][4], \
                                ...
                                arr_min[k][4],arr_min[k][5],arr_min[k][6],arr_min[k][7],arr_min[k][8],arr_wells[m][8],arr_wells[m][9],arr_wells[m][10], \
                                arr_wells[m][11]]
    	...
    	#[id_скважины,линия/имя скважины,x,y,z,глубина скважины,объект,участок,номер пробы,id пробы,код типа пробы,интервал глубины от,
    	#интервал глубины до,возраст,порода,находки МСА,тип скважины, статус документирования,состаяние опробования,
    	#тип документации]
	return def_united_array
# функция на входе берет массив в формате[id_скважины,линия/имя скважины,x,y,z,глубина скважины, объект,участок,
#номер пробы,id пробы,тип пробы,интервал глубины от, интервал глубины до, возраст, порода, находки МСА,
...
def create_final_array(arr_min,arr_strat_lit):
	...
	if(len(arr_strat_lit)==0):#если массив с данными по литостратиграфии пуст, вернуть массив с пустыми значениями по литостратиграфии
                ...
                while(k<len(arr_min)):#перебор элементов первого листа (массива точек c минералогией)
                        ...
                        arr_min[k][11],arr_min[k][12],arr_min[k][13],arr_min[k][14],arr_min[k][15],"","","","", \
                        arr_min[k][16],arr_min[k][17],arr_min[k][18],arr_min[k][19]]
            		def_united_array.append(curr_array) #добавить в общий массив точек
                        k+=1
	...
	while(k<len(arr_min)):#перебор элементов первого листа (массива точек c минералогией)
    		...
    		while(m<len(arr_strat_lit)):#перебор элементов второго листа (массива с данными литостратиграфии) 
				...
				curr_array = [arr_min[k][0],arr_min[k][1],arr_min[k][2], \
                                ...
                                arr_min[k][7],arr_min[k][8],arr_min[k][9],arr_min[k][10], \
                                arr_min[k][11],arr_min[k][12],arr_min[k][13],arr_min[k][14],arr_min[k][15],\
                                arr_strat_lit[m][1],arr_strat_lit[m][2], \
        		...
        		if(m==len(arr_strat_lit))and(not find_stratolit):
            			...
            			def_united_array.append(curr_array) #добавить в общий массив точек
    		k=k+1
    	#вернуть массив в формате [id_скважины,линия/имя скважины,x,y,z,глубина скважины,объект,участок,
    	#номер пробы,id пробы,тип пробы,интервал глубины от, интервал глубины до, возраст, порода, находки МСА,
    	...
    	#тип документации]
    	return def_united_array
#функция на входе берет объединенный массив
#заменяет пустые значения возрастов(взятых с листа минералогии) и записывает расчитанные значения 
def replace_old(def_final_array):
        gp=arcgisscripting.create(9.3)#создать объект геопроцессинга версии 9.3
        for input in def_final_array:#перебор проб
                if input[13]=='Нет данных':#если нет данных по возрасту
                        mark_from = float(input[11])#интервал пробы от
                        mark_to = float(input[12])#интервал пробы до
                        arr_mark_litostrat_roof = input[16].split("/")#массив абсолютных отметок для кровель пластов
                        arr_mark_litostrat_sole = input[17].split("/")#массив абсолютных отметок для подошв пластов
                        arr_strat = input[18].split("/")#массив стратиграфий пластов
                        arr_lit = input[19].split("/")#массив литологии пластов
                        if(len(arr_strat)==1)or(len(arr_mark_litostrat_roof)==1)or(len(arr_mark_litostrat_sole)==1):#если нет данных по стратиграфии или пластам для текущей пробе
                                arcpy.AddMessage("нет данных по стратиграфии и(или) пластам для текущей пробы")
                                continue#поскольку нет данных по стратиграфии и(или) пластам по текущей пробе, перейти к следующей пробе
                        m=0
                        while(m<len(arr_strat)-1):#перебор стратиграфических пластов для текущей пробы, не обрабатывать последний элемент массива, поскольку 1/2/3 даст ['1','2','3','']
                                if(mark_from==float(arr_mark_litostrat_roof[m]))and(mark_to==float(arr_mark_litostrat_sole[m])):
                                        ...
                                        input[13]=arr_strat[m]+" (вычислено)"#записать в поле стратиграфии вычисленное значение возраста
                                        if input[14]=='Нет данных':input[14]=arr_lit[m]+" (вычислено)"#записать вычисленное значение литологии
                                        break
                                        ...
                                        input[13]=arr_strat[m]+" (вычислено)"
                                        if input[14]=='Нет данных':input[14]=arr_lit[m]+" (вычислено)"
                                        break
                                elif(mark_from<=float(arr_mark_litostrat_roof[m]))and(mark_to>=float(arr_mark_litostrat_sole[m])):
                                        ...
                                        if input[14]=='Нет данных':input[14]=arr_lit[m]+" (вычислено)"
                                        break
                                #случай 4 не разу не встретился, поскольку проба почти всегда совпадает по верхней границе
...
def write_to_layer(lay, C_System, array_map, atr_min):
        ...
        if fc_path.count("-") or fc_name.count("-"):
                gp.AddMessage(" '-' Недопустимый символ в названии файла, слой не создан")
        else:
                try:
                        ...
                        gp.AddField(lay,"название_скважины","STRING","","",100)#атрибут названия скважин
                        ...
                        gp.AddField(lay,"возраст","STRING","","",50)#атрибут - возраст (проба)
                        gp.AddField(lay,"порода","STRING","","",150)#атрибут - порода (проба)
                        ...
                        while(k<len(array_map)): #создание точечного слоя
                                ...
                                old_name = str(array_map[k][13])
                                rock_name = str(array_map[k][14])
                                mineralogy = str(array_map[k][15])
                                arr_min = mineralogy.split("/")
                                abs_litostrat_roof = str(array_map[k][16])
                                abs_litostrat_sole = str(array_map[k][17])
                                stratigraphy = str(array_map[k][18])
                                lithology = str(array_map[k][19])
                                type_tn = str(array_map[k][20])
                                stat_doc = str(array_map[k][21])
                                stat_test = str(array_map[k][22])
                                type_doc = str(array_map[k][23])
                                ...
                                feat.SetValue("возраст",old_name)
                                feat.SetValue("порода",rock_name)
                        ...
                        gp.AddMessage(str(sys.exc_info()[0]))
                        gp.AddMessage(str(traceback.format_exc()))#выводит строку кода, сгенерировшую исключение
if __name__== '__main__':#если модуль запущен, а не импортирован
        ...
        output_workspace = ConversionUtils.gp.GetParameterAsText(2)#рабочая область
        ...
        for input in excel_list_files:#перебор excel-файлов
                ...
                #получить объект excel-листов для текущего excel-файла
                ...
                get_excel_list(input, List_point,List_strat_lit,List_code_strat,List_code_lit,List_mineral, \
                ...
                ver_lists = verify_lists(obj_point, obj_strat_lit, obj_code_strat, obj_code_lit, obj_mineral_list,obj_code_type_test, \
                obj_code_type_TN,obj_code_status_document,obj_code_status_test,obj_code_type_document)
                if not ver_lists:
                ...
                if(len(wells_array)==0):#если массив точек наблюдений пуст 
                ...
                if(len(code_status_test)==0):#если массив кодов состояния опробования пуст - завершить работу программы
                        ARCPY.AddMessage("Лист с кодами состояния опробования пуст для текущего excel-файла")
                        continue#перейти к обработке следующего excel-файла
                if(Full_report == "true"):
                        ARCPY.AddMessage("В формате:")
                        k=0
                ...
                if (len(non_empty)==0):
                        ARCPY.AddMessage("Нет данных по минералогии для текущего excel-файла")
                        continue#перейти к обработке следующего excel-файла
                ...
                if(len(code_type_test)==0):#если массив кодов с типами проб пуст - завершить работу программы
                        ARCPY.AddMessage("Лист кодов типов проб пуст для текущего excel-файла")
                        continue#перейти к обработке следующего excel-файла
                if(Full_report == "true"):#выводить полный отчет
                        if(len(code_type_test)>10):
                                ...
                                ARCPY.AddMessage("-----------------------------------------------------------------------------------")
                #получить массив, где вместо кодов типов пород - реальные названия, такие как шлих, шлам и т.д.
                arr_mineral = replacement_code_test(non_empty,code_type_test,code_strat,code_lit)
                ...
                if(len(united_array)==0):#если объединенный массив пуст
                        ARCPY.AddMessage("Массив данных скважин и минералогии пуст для текущего excel-файла")
                        continue#перейти к обработке следующего excel-файла
                if(Full_report == "true"):#выводить полный отчет
                        ARCPY.AddMessage("В формате:")
                        if(len(arr_mineral)>5):
                                ...
                                while(k<5):
                                        ...
                                        " "+str(united_array[k][18])+" "+str(united_array[k][19]))
                                        k+=1
                                ARCPY.AddMessage("-----------------------------------------------------------------------------------")
                ...
                if(Full_report == "true"):#выводить полный отчет
                        ...
                        while(m<24):
                                ...
                                m+=1
                        ARCPY.AddMessage(stroka)
                #функция заменяет пустые значения возрастов(взятых с листа минералогии) и записывает расчитанные значения
                try:
                        replace_old_arr = replace_old(final_array)
                except:
                        gp.AddMessage(str(sys.exc_info()[0]))
                        gp.AddMessage(traceback.format_exc)#выводит строку кода, сгенерировшую исключение
                #вывести полученный массив в картографический слой 
                write_to_layer(outdata, Coordinate_System, replace_old_arr, list_atributes)
