...
def get_all_files(directory):#получить все файлы в заданной директории(включая файлы во вложенных директориях, если такие есть)
    current_file_list=[]
    objects = listdir(directory)#объекты в текущем каталоге
    for obj in objects:#перебор объектов
        if isfile(join(directory,obj)):
            current_file_list.append(directory+"*"+obj)#если объект файл
        elif isdir(join(directory,obj)):
            current_file_list+=get_all_files(join(directory,obj))#если объект директория, прочитать эту директорию в следующей рекурсии
    return current_file_list
def select_type_files(list_files,extension_file):#выбрать файлы с указанным расширением
    arr_path_file=[]
    select_files = filter(lambda x:x.endswith("."+extension_file),list_files)#выбрать файлы с указанным расширением
    for f in select_files:arr_path_file.append(f.split("*"))#добавить в выходной массив текущую пару - [путь,имя файла]
    return arr_path_file
...
def read_excel_list(excel_file,List_point,List_strat_lit,List_code_strat,List_code_lit,List_GIS,List_code_type_TN,List_code_status_document,List_code_type_document):
    ...
    excel_book = open_workbook(excel_file[0]+"\\"+excel_file[1],formatting_info=True)#получить объект книги excel
    array_list = excel_book.sheet_names()#получить список названий листов excel
    ARCPY.AddMessage(array_list[0])
    k=0
...
if __name__== '__main__':#если модуль запущен, а не импортирован
    #-------------------------------входные данные------------------------------------
    Folder_with_exel_files = ARCPY.GetParameterAsText(0)#входная папка содержащая excel-файлы
    ...
    #---------------------------------------------------------------------------------
    all_files=get_all_files(Folder_with_exel_files)#получить все файлы в директории Folder_with_exel_files
    select_files = select_type_files(all_files,"xls")#получить файлы с расширением xls
    ARCPY.AddMessage("Количество excel-файлов: "+str(len(select_files)))
    for ef in select_files:ARCPY.AddMessage(ef[1])
    ARCPY.AddMessage("-----------------------------------------------------------------------------------")
    map_array=[]#массив для вывода всех ТН в выходной картографический слой
    for current_excel in select_files:#перебор excel-файлов
        ARCPY.AddMessage("-----------------------------------------------------------------------------------")
        ARCPY.AddMessage(u'Excel-файл: '+current_excel[1])
        ...
        read_excel_list(current_excel,List_point,List_strat_lit,List_code_strat,List_code_lit,List_GIS,\
