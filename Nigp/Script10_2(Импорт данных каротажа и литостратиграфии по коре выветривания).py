...
#Функция в качестве входных данных берет массив кодов стратиграфии и массив с символами возраста для коры выветривания
#и возвращает массив в формате [код стратиграфии, наименование стратиграфии, кора выветривания(Y) или не кора(N)]
def calc_type_rock(arr_strat, country):
	...
	while (k<len(arr_strat)):#перебор массива с кодами стратиграфии
		m=0
		b=False #переменная определяет были ли найден символы геологического периода, соответствующего коре выветривания
		while (m<len(country)):#перебор массива с символами для коры выветривания
			if(arr_strat[k][1].count(country[m])):#если была обнаружена вмещающая порода
				#добавить в массив кору выветривания
				def_code_strat_new.append([arr_strat[k][0],arr_strat[k][1],"Y"])
				b=True #были найдены породы для коры выветривания
				break#выйти из внутреннего цикла
			m+=1
		if (b==False):#если не было совпадений с корой выветривания
			#добавить в массив <<не кору выветривания>> 
			def_code_strat_new.append([arr_strat[k][0],arr_strat[k][1],"N"])
		k+=1
	#вернуть массив в формате [код стратиграфии, обозначение стратиграфии, код отложений (кора выветривания или другие породы)]
...
#и массив [код стратиграфии, наименование стратиграфии, код коры ввыветривания(Y) или не кора(N)]
#и возвращает массив [код скважины, abs кровли коры, abs подошвы коры, мощность коры]
def calc_width_blocking(arr_width, code_strat):
	...
	while(k<len(arr_width)):#перебор массива скважин формата [код точки, кровли пластов, коды стратиграфий, коды литологий]
		find_fraction=False#логическая переменная определяет найдена ли кора выветривания
		...
		while(m<len(arr_code)-1):
			n=0
			if(find_fraction):break#выйти из этого цикла если кора выветривания уже была обнаружена. Перейти к следующей скважине
			#перебор всех кодов стратиграфий массива [код стратиграфии, наименование стратиграфии, код коры(Y) или не коры(N)]
			while(n<len(code_strat)):
				#если код стратиграфии текущего пласта совпадает с кодом из массива типов пород(кора/не кора)
				...
				if(cod1==cod2):#если коды совпали
					if(code_strat[n][2]=="Y"):#если этому коду соответствует кора выветривания
                                                #массив для текущей скважины [код скважины, abs кровли пласта коры, abs подошвы пласта коры, мощность коры]
						current_array=[arr_width[k][0],arr_roof[m],arr_sole[m],float(arr_sole[m])-float(arr_roof[m])]
			...
			m+=1
		k+=1
	return def_arr_width_blocking #вернуть массив [код скважины, abs кровли коры выветривания, abs подошвы коры выветривания, мощность коры]
...
#и массив [код скважины, abs кровли коры, abs подошвы коры, мощность коры]
#возвращает массив [id точки,линия/номер,x,y,z,глубина,объект,участок,глубина/глубина...,значение ГИС/значение ГИС...,
#мощность перекрывающих отложений, abs кровли коры, abs подошвы коры, мощность коры]
def united_width_blocking(un_array, block_array):
	def_united_final_arr=[]#общий массив
	if(len(block_array)==0):#если массив с данными о коре пуст, вернуть массив с пустыми значениями для коры
                k=0
                while(k<len(un_array)):#перебор элементов объекдиненного массива с точками наблюдений и ГИС
                        ...
                        un_array[k][5],un_array[k][6],un_array[k][7],un_array[k][8],un_array[k][9],"-1.0","-1.0","-1.0"]
                        def_united_final_arr.append(curr_array)#записать в общий массив информацию о текущей скважине
                        k+=1
                #возвращает массив с пустыми значениями по коре[id точки,линия/номер,x,y,z,глубина скважины,
                #объект,участок,глубина/глубина...,значение ГИС/значение ГИС...,"-1.0","-1.0","-1.0"]
	...
	while(k<len(un_array)):#перебор элементов массива [id точки,линия/номер,x,y,z,глубина,объект,участок,глубина/глубина...,значение ГИС/значение ГИС...]
                find_m_bool=False#логическая переменная определяет, вычислены ли для текущей скважины данные по коре
		curr_array=[]
		cod1=float(un_array[k][0])#код точки наблюдений первого массива
		m=0
		while(m<len(block_array)):#перебор элементов массива [код скважины, abs кровли коры, abs подошвы коры, мощность коры]
			...
			if(cod1==cod2):#если найдено совпадений кодов
				...
				curr_array=[un_array[k][0],un_array[k][1],un_array[k][2],un_array[k][3],un_array[k][4],un_array[k][5],un_array[k][6],un_array[k][7],un_array[k][8],un_array[k][9],block_array[m][1],block_array[m][2],block_array[m][3]]
				def_united_final_arr.append(curr_array)#записать в общий массив информацию о текущей скважине
				find_m_bool=True#для текущей скважины вычислены параметры коры
			m+=1
			#если достигнут конец цикла и при этом не найдено совпадения кодов(т.е. для текущей скважины отсутствуют данные по коре)
			if(m==len(block_array))and(not find_m_bool):
                                #Все равно добавить к финальному массиву данные по текущей скважине без данных о коре
                                curr_array=[un_array[k][0],un_array[k][1],un_array[k][2],un_array[k][3],un_array[k][4],un_array[k][5],un_array[k][6],un_array[k][7],un_array[k][8],un_array[k][9],"-1.0","-1.0","-1.0"]
                                def_united_final_arr.append(curr_array)#записать в общий массив информацию о текущей скважине без сведений о коре
		k+=1
	#возвращает массив [id точки,линия/номер,x,y,z,глубина скважины,объект,участок,глубина/глубина...,значение ГИС/значение ГИС...,abs кровли коры, abs подошвы коры, мощность коры]
	return def_united_final_arr
...
#abs кровли коры, abs подошвы коры, мощность коры]
...
def united_data_litostrat(fin_array,arr_stratolit):
        ...
        if(len(arr_stratolit)==0):#если массив с данными по литостратиграфии пуст, вернуть массив с пустыми значениями по литостратиграфии
                ...
                #глубина/глубина...,значение ГИС/значение ГИС...,abs кровли коры, abs подошвы коры, мощность коры]
                while(k<len(fin_array)):
                        ...
                        fin_array[k][11],fin_array[k][12],"","","",""]
                        un_array_litostrat.append(curr_array)
                        k+=1
                #вернуть массив с пустыми значениями литостратиграфии
                #[id точки,линия/номер,x,y,z,глубина скважины,объект,участок,глубина/глубина...,значение ГИС/значение ГИС...,
                #abs кровли коры, abs подошвы коры, мощность коры, abs кровли/abs кровли/abs кровли, abs подошвы/abs подошвы/abs подошвы,
                #Jmh/Jsn/Ol/, алевролит/известняк/доломит/]
        ...
        while(k<len(fin_array)):
                ...
                while(m<len(arr_stratolit)):
                        ...
                        if(cod1==cod2):#если найдено совпадение кодов
                                ...
                                curr_array=[fin_array[k][0],fin_array[k][1],fin_array[k][2],fin_array[k][3],fin_array[k][4],\
                                fin_array[k][5],fin_array[k][6],fin_array[k][7],fin_array[k][8],fin_array[k][9],fin_array[k][10],\
                                fin_array[k][11],fin_array[k][12],arr_stratolit[m][1],arr_stratolit[m][2],arr_stratolit[m][3],arr_stratolit[m][4]]
                                un_array_litostrat.append(curr_array)
...
def write_to_layer(lay, C_System, array_map):
	...
	gp.AddField(OutputFC,"value_logging","STRING","","",1000)#атрибут для значений ГИС
        ...
        gp.AddField(OutputFC,"abs_roof_bark","FLOAT",15,15)#атрибут кровли коры
        gp.AddField(OutputFC,"abs_sole_bark","FLOAT",15,15)#атрибут подошвы коры
        gp.AddField(OutputFC,"depth_bark","FLOAT",15,15)#мощность коры
	...
	while(k<len(array_map)): #создание точечного слоя
    		...
    		abs_roof_bark = float(array_map[k][10])#атрибут кровли коры
                abs_sole_bark = float(array_map[k][11])#атрибут подошвы коры
                depth_bark = float(array_map[k][12])#мощность коры
    		
    		abs_litostrat_roof = str(array_map[k][13])#глубины кровли пластов
                abs_litostrat_sole = str(array_map[k][14])#глубины подошвы пластов
    		stratigraphy = str(array_map[k][15])#стратиграфия
    		lithology = str(array_map[k][16])#литология
		...
		feat.SetValue("y",y)
    		...
    		feat.SetValue("abs_roof_bark",abs_roof_bark)
                feat.SetValue("abs_sole_bark",abs_sole_bark)
...
if __name__== '__main__':#если модуль запущен, а не импортирован
        ...
        code_contain = ARCPY.GetParameterAsText(7).encode('utf-8')#Индексы возрастов коры выветривания
        ...
        arr_country_rock = code_contain.split(";")#получить массив индексов возрастов коры выветривания
        ...
        if(Full_report == "true"):#выводить полный отчет
                if(len(code_strat)>10):
                        ...
                        ARCPY.AddMessage("...........")
                        ARCPY.AddMessage("-----------------------------------------------------------------------------------")
        #получить массив с кодами стратиграфии и типами пород(кора/не кора)
        ...
        if(len(arr_view_strat_lit)==0):#если нет скважин со стратолитологическим описанием
                ...
                if(Full_report == "true"):#выводить полный отчет
                        ...
                        ARCPY.AddMessage("-----------------------------------------------------------------------------------")     
                arr_width_blocking=calc_width_blocking(arr_single_width, code_strat_new)#вычислить abs кровли и подошвы коры выветривания
        ARCPY.AddMessage("Количество скважин с корой выветривания: "+str(len(arr_width_blocking)))
        if(len(arr_width_blocking)==0):#если нет скважин с корой выветривания
                ARCPY.AddMessage("Массив скважин с корой выветривания пуст")
        else:#если есть скважины с корой выветривания
                if(Full_report == "true"):#выводить полный отчет
                        ARCPY.AddMessage("В формате:")
                        k=0
                        while(k<len(arr_width_blocking)):
                                ARCPY.AddMessage(str(arr_width_blocking[k][0])+" "+str(arr_width_blocking[k][1])+\
                                " "+str(arr_width_blocking[k][2])+" "+str(arr_width_blocking[k][3]))
                                k+=1
                        ARCPY.AddMessage("-----------------------------------------------------------------------------------")
        ...
        #получить объединенный массив [id точки,линия/номер,x,y,z,глубина скважины,объект,участок,глубина/глубина...,значение ГИС/значение ГИС...,
        #abs кровли коры, abs подошвы коры, мощность коры]
        final_array=united_width_blocking(united_array, arr_width_blocking)
        ARCPY.AddMessage("Количество элементов объединенного массива с данными по коре: "+str(len(final_array)))
        if(len(final_array)==0):#если массив пуст
                ARCPY.AddMessage("Объединенный массив с данными по коре пуст, программа остановлена")
                raise SystemExit(1)#завершить работу программы
        if(Full_report == "true"):#выводить полный отчет
                ARCPY.AddMessage("В формате:")
        ...
        if(Full_report == "true"):#выводить полный отчет
                ...
                ARCPY.AddMessage("abs кровли коры выветривания: "+str(map_array[0][10]))
                ARCPY.AddMessage("abs подошвы коры выветривания: "+str(map_array[0][11]))
                ARCPY.AddMessage("мощность коры выветривания: "+str(map_array[0][12]))
                
                ARCPY.AddMessage("глубины залегания кровли пластов: "+str(map_array[0][13]))
                ARCPY.AddMessage("глубины залегания подошвы пластов: "+str(map_array[0][14]))
                ARCPY.AddMessage("стратиграфия пластов: "+str(map_array[0][15]))
                ARCPY.AddMessage("литология пластов: "+str(map_array[0][16]))
                ARCPY.AddMessage("-----------------------------------------------------------------------------------")
